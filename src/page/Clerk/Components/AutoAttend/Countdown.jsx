...
const Countdown = (props) => {
  ...
  useEffect(() => {
    if (new Date(new Date(new Date(admitT) - time)).getTime() >= 0) {
      setTimediff(
        new Date(new Date(new Date(admitT) - time))
          .toISOString()
          .replace('T', ' ')
          .replace(/\..*/, '')
          .substr(14)
      ...
      setTimediff(
        new Date(new Date(new Date(lateT) - time))
          .toISOString()
          .replace('T', ' ')
          .replace(/\..*/, '')
          .substr(14)
      ...
      if (
        timediff == '00:01' ||
        new Date(new Date(new Date(lateT) - time)).getTime() <= 0
      ) {
    ...
    <div className="flex flex-1 justify-center w-full">
      {new Date(new Date(new Date(admitT) - time)).getTime() > 0 ? ( //아직 인정시간일 때
        <div>
          ...
          <div>
            지각:{' '}
            {new Date(new Date(new Date(lateT) - new Date(admitT)))
              .toISOString()
              .replace('T', ' ')
              .replace(/\..*/, '')
              .substr(14)}
